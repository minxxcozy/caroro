...
class Traffic_control:
    def __init__(self):
        rospy.init_node("traffic_signal_publisher")
        self.signal_pub = rospy.Publisher("/traffic_status", Bool, queue_size=1)
        rospy.Subscriber("/GetTrafficLightStatus", GetTrafficLightStatus, self.traffic_CB)
        self.signal_msg = Bool()
