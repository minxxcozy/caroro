...
from rotary_mission import rotary_mission
...
class MissionController:
    def __init__(self):
        ...
        rospy.Subscriber('obstacle_run_cmd', Float64, self.obstacle_run_callback)  # 장애물 명령 구독
        ...
        self.stop_detect_delay = rospy.Duration(2)  # 2초 동안 무시
        
        self.printed_stop_count_zero = False
    ...
    def traffic_light(self, frame):
        if not self.printed:
            print("traffic light mission start!")
            self.printed = True
    ...
    def obstacle_run_callback(self, msg):
        val = msg.data
        if val == 1.0:
            rospy.loginfo("정적 장애물 대응")
            self.motor_speed_pub.publish(Float64(1.0)) 
        elif val == 2.0:
            rospy.loginfo("동적 장애물 대응")
            self.motor_speed_pub.publish(Float64(0.5))  
        elif val == 3.0:
            rospy.loginfo("정상 주행")
            self.motor_speed_pub.publish(Float64(self.current_speed))
    ...
    def image_CB(self, msg):
        ...
        if self.stop_line_count == 0 and not self.printed_stop_count_zero:
            rospy.loginfo(f"정지선 누적 감지 수: {self.stop_line_count}")
            self.printed_stop_count_zero = True
        ...
        # 미션 수행
        if self.current_mission == 1:  # 로터리 미션이면
            rotary_mission(frame)
