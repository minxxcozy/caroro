...
class Rotary:
    def __init__(self):
        ...
        # /lane_cmd 구독
        rospy.Subscriber("/line_cmd", Int32, self.line_cmd_CB)
        self.line_cmd = None    # 차선 정보 저장
    ...
    def lidar_CB(self, msg):
        ...
        for i in range(len(self.scan_msg.ranges)):
            ...
            if 0 < dist < 2:
                # 2차선 진입
                if self.line_cmd == 2: 
                    # Stop 조건: -10도 ~ 45도 범위에 장애물이 감지되면 플래그를 True로 설정
                    if -10 <= deg <= 45:
                        stop_condition_met = True
                
                    # Go 조건: -30도 근처(오차범위 0.5도)에 장애물이 감지되면 플래그를 True로 설정
                    if abs(deg + 30) < 0.5:
                        go_condition_met = True
                
                # 1차선 진입
                elif self.line_cmd == 1: 
                    if -30 <= deg <= 40:
                        stop_condition_met = True
                    if abs(deg + 40) < 0.5:
                        go_condition_met = True
